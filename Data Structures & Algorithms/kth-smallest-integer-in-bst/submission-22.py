        ...
        while curr:
            if not curr.left:
                ...
                # Second time/After clearing everything on the left of "Current" Node/not prev node cuz prev = curr.left
                    ...
                    k -= 1
                    if k == 0:
                        res = curr.val # Unintuitive, but when we reach this last right end node, we must have gone through this node in the first if block" to "this node was either processed in the first if block (as a leaf) OR in the first if block OR when its predecessor's right end is returning back to this node

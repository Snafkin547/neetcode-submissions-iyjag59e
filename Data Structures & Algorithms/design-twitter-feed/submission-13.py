...
class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        # Create maxHeap if bigger than 10 (use negative of negative, so the resulting is smallest first)
        if len(self.userMap[userId]) >= 10:
            for followeeId in umap_copy:
                ...
                if followeeId in self.tweetMap: # Add just the latest tweet of the poster
                    ...
                    maxHeap.push([-time, tweetId, followeeId, idx - 1])
                    if len(mh.heap) > 10:
                        mh.pop()
            while maxHeap.heap:
                time, tweetId, followeeId, idx = maxHeap.pop()
                mh.push([-time, tweetId, followeeId, idx])
        else:
            for followeeId in umap_copy:
                if followeeId in self.tweetMap: # Add just the latest tweet of the poster
                    idx = len(self.tweetMap[followeeId]) - 1
